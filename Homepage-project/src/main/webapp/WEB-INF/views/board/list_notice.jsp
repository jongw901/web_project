			...
			<form role="form" method="get">
				<table class="type">
					...
					<c:forEach items="${list}" var="list">
						<tr>
							<td><a href="/board/readView_notice?bno=${list.bno}&page=${scri.page}&
															perPageNum=${scri.perPageNum}&
															searchType=${scri.searchType}&
															keyword=${scri.keyword}"><c:out value="${list.title}" /></a></td>
							<td><c:out value="${list.writer}" /></td>
							<td class="da_d"><fmt:formatDate value="${list.regdate}" pattern="MM-dd" /></td>
							<td class="hi_h"><c:out value="${list.hit}"/></td>
						</tr>
					</c:forEach>
				</table>
				<hr />
					...
					<td>
						<ul>
							<c:if test="${pageMaker.prev}">
								<li class="page_btn"><a
									href="list_notice${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
							</c:if>
							<c:forEach begin="${pageMaker.startPage}"
								end="${pageMaker.endPage}" var="idx">
								<li class="page_btn ${pageMaker.cri.page == idx ? "active" : "" }"><a
									href="list_notice${pageMaker.makeSearch(idx)}">${idx}</a></li>
							</c:forEach>
							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li class="page_btn"><a
									href="list_notice${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
							</c:if>
						</ul>
					</td>
				</tr>
				...
				<tr style="margin: 0px auto;">
					<td style="padding-top: 20px;">
						...
						<input class="searchContent" type="text" name="keyword" id="keywordInput" value="${scri.keyword}" placeholder= "검색어를 입력해 주세요."; />
						<button class="searchBtn" id="searchBtn" type="button">검색</button>
						<script>
							$(function() {
								$('#searchBtn').click(
										function() {
											self.location = "list_notice" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val()+ "&keyword="+ encodeURIComponent($('#keywordInput').val());
											});
								});
						</script>
					</td>
